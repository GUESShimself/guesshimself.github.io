(() => {
  ...
  var stickyClasses = ["fixed", "h-14"];
  var unstickyClasses = ["absolute", "h-20"];
  var stickyClassesContainer = ["border-neutral-300/50", "bg-white/80", "dark:border-neutral-600/40", "dark:bg-neutral-900/60", "backdrop-blur-2xl"];
  var unstickyClassesContainer = ["border-transparent"];
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    allTriggers.forEach((trigger) => {
      ...
      function handleActivation() {
        const targetId2 = this.getAttribute("data-target");
        const contentElement = document.getElementById(targetId2);
        const isBottomTrigger = this.classList.contains("expandable-bottom-trigger");
        const topTrigger = document.querySelector(`.expandable-trigger[data-target="${targetId2}"]`);
        const topTriggerPosition = topTrigger ? topTrigger.getBoundingClientRect().top + window.pageYOffset : null;
        const allTriggersForTarget = document.querySelectorAll(`[data-target="${targetId2}"]`);
        ...
        if (contentElement.classList.contains("expanded")) {
          ...
          allTriggers.forEach((otherTrigger) => {
            ...
            if (otherId !== targetId2 && document.getElementById(otherId)?.classList.contains("expanded")) {
